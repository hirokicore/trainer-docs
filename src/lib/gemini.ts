...
/**
 * Gemini 整形が失敗した場合の最低限フォールバック。
 * 口語・断片文のままでも「・」始まりの体裁に揃える。
 */
export function formatFallbackNotes(text: string): string {
...
以下のユーザー入力を、契約書の特記事項条文として整形してください。

【ユーザー入力】
${trimmed}

【厳守事項】
- 必ず「・」で始まる1行の箇条書きとして出力すること
- 「〜とする」「〜するものとする」「〜に従うものとする」など契約書調の文末にすること
- 口語・メモ書き・断片表現は必ず正式な条文の文体に書き直すこと（元の意図は変えない）
- 文脈に応じて甲（クライアント）・乙（トレーナー）の主語を補うこと
- プレーンテキストのみ（マークダウン・条番号・複数行 不可）
- 長さは1〜2文に収めること

【変換例】
入力：「極力、要望に応じてトレーナー変更に対応」
出力：「・クライアント（甲）から要望があった場合、乙は可能な範囲でトレーナーの変更に対応するものとする。」

整形後の1行テキストのみを出力してください。前置き・説明は不要です。
`.trim();
